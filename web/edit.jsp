	...
	<div id="templatemo_content">
      ...
      <div id="templatemo_main_coontent">
		...
		<div class="box"></div> <!-- end of date box -->
                ...
                <div class="post_body">
                    <p> <h2><fmt:message bundle="${msgs}" key="edit.edit"/></h2></p>
                        <table border="0" cellpadding="15">
                        <td>
                            <p><font><fmt:message bundle="${msgs}" key="edit.surname"/></font></p> <input name="surname" pattern="[a-zA-Zа-яА-Я]+" type="text" required value=<%=user.getSurname()%> >
                            <p><font><fmt:message bundle="${msgs}" key="edit.name"/></font> </p><input name="name" pattern="[a-zA-Zа-яА-Я]+" type="text" required value=<%=user.getName()%>>
                            <p><font><fmt:message bundle="${msgs}" key="edit.date"/></font></p> <input name="birthday" type="date" value=<%=user.getBirthday()%>>
                            <p><font><fmt:message bundle="${msgs}" key="edit.serialNumber"/></font></p> <input name="serialnumber" pattern="[A-Z]+" type="text" value=<%=user.getSerialNumber()%>>
                        </td>
                        <td>
                            <p><font><fmt:message bundle="${msgs}" key="edit.passportNumber"/></font> </p><input name="passport" pattern="[0-9]+" type="text" value=<%=user.getPassportNumber()%>> 
                            <p><font><fmt:message bundle="${msgs}" key="edit.validTo"/></font> </p><input name="valid"  type="date" value=<%=user.getValid()%>>
                            <p><font><fmt:message bundle="${msgs}" key="edit.login"/></font> </p><input name="login" pattern="[a-zA-Z0-9]+" readonly type="text" required value=<%=user.getLogin()%>>
                            <p><font><fmt:message bundle="${msgs}" key="edit.password"/></font> </p><input name="password"  type="text" required value=<%=user.getPassword()%>>
                         </td>
                        </table>
                    ...
                    <p><font class="error"><c:out value="${error}"/></font></p>
                        ...
                        <div class="post_submit">
                            <input type="submit" class="autoriz" value=<fmt:message bundle="${msgs}" key="edit.button"/>>
                        </div>
